<div class="modal fade" id="editPromoModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content bg-white dark:bg-slate-900 w-full rounded-3xl shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden" style="border: none;">
            ...
            <form action="{{ route('admin-cabang.promo.update', $promo->id_promo) }}" method="POST">
                @csrf 
                @method('PUT')
                <div class="modal-body p-6">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                        <div class="md:col-span-6">
                            ...
                            <input type="text" name="nama_voucher" class="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-borma-purple dark:focus:ring-borma-yellow text-slate-800 dark:text-white transition-all" value="{{ $promo->nama_voucher }}" required>
                        </div>
                        <div class="md:col-span-6">
                            ...
                            <input type="text" name="kode_voucher" class="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-borma-purple dark:focus:ring-borma-yellow text-slate-800 dark:text-white transition-all" value="{{ $promo->kode_voucher }}">
                        </div>
                        <div class="md:col-span-6">
                            ...
                            <select name="id_produk_pemicu" class="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-borma-purple dark:focus:ring-borma-yellow text-slate-800 dark:text-white transition-all" required style="cursor: pointer;">
                                ...
                                @foreach($produkList as $pc)
                                    <option value="{{ $pc->produk->id_produk }}" @selected($pc->produk->id_produk == $promo->id_produk_pemicu)>{{ $pc->produk->nama_produk }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="md:col-span-3">
                            ...
                            <input type="number" name="kuantitas_pemicu" class="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-borma-purple dark:focus:ring-borma-yellow text-slate-800 dark:text-white transition-all" min="1" value="{{ $promo->kuantitas_pemicu }}" required>
                        </div>
                        <div class="md:col-span-3">
                            ...
                            <input type="number" name="kuota_promo" class="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-borma-purple dark:focus:ring-borma-yellow text-slate-800 dark:text-white transition-all" min="1" value="{{ $promo->kuota_promo }}" required>
                        </div>
                        <div class="md:col-span-6">
                            ...
                            <select name="id_produk_hadiah" class="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-borma-purple dark:focus:ring-borma-yellow text-slate-800 dark:text-white transition-all" style="cursor: pointer;">
                                ...
                                @foreach($produkList as $pc)
                                    <option value="{{ $pc->produk->id_produk }}" @selected($pc->produk->id_produk == $promo->id_produk_hadiah)>{{ $pc->produk->nama_produk }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="md:col-span-6">
                            <label class="block text-sm font-medium text-slate-700 dark:text-white/80 mb-2">Kuantitas Hadiah (pcs)</label>
                            <input type="number" name="kuantitas_hadiah" class="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-borma-purple dark:focus:ring-borma-yellow text-slate-800 dark:text-white transition-all" min="0" value="{{ $promo->kuantitas_hadiah }}">
                        </div>
                        <div class="md:col-span-4">
                            <label class="block text-sm font-medium text-slate-700 dark:text-white/80 mb-2">Potongan Harga (Rp)</label>
                            <input type="number" name="potongan_harga" class="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-borma-purple dark:focus:ring-borma-yellow text-slate-800 dark:text-white transition-all" min="0" value="{{ $promo->potongan_harga }}">
                        </div>
                        <div class="md:col-span-4">
                            <label class="block text-sm font-medium text-slate-700 dark:text-white/80 mb-2">Min. Transaksi (Rp)</label>
                            <input type="number" name="min_transaksi" class="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-borma-purple dark:focus:ring-borma-yellow text-slate-800 dark:text-white transition-all" min="0" value="{{ $promo->min_transaksi }}">
                        </div>
                        <div class="md:col-span-4">
                            <label class="block text-sm font-medium text-slate-700 dark:text-white/80 mb-2">Maks. Potongan (%)</label>
                            <input type="number" name="max_promo" class="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-borma-purple dark:focus:ring-borma-yellow text-slate-800 dark:text-white transition-all" min="0" max="100" value="{{ $promo->max_promo }}">
                        </div>
                        <div class="md:col-span-6">
                            ...
                            <input type="date" name="tanggal_mulai" class="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-borma-purple dark:focus:ring-borma-yellow text-slate-800 dark:text-white transition-all" value="{{ $promo->tanggal_mulai->format('Y-m-d') }}" required>
                        </div>
                        <div class="md:col-span-6">
                            ...
                            <input type="date" name="tanggal_berakhir" class="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-borma-purple dark:focus:ring-borma-yellow text-slate-800 dark:text-white transition-all" value="{{ $promo->tanggal_berakhir->format('Y-m-d') }}" required>
                        </div>
                    </div>
                </div>
